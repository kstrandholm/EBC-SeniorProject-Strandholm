...
var registrantID = (from reg in Registrants
                   where reg.FirstName == "Kelly" && reg.LastName == "Strandholm"
                   select reg.ID).Single();

var talkIDs = (from talk in Talks
              where talk.Title.Contains("ow")
              select talk.ID).ToList();

var talkInterests = new List<TalkInterest>();

foreach (var talk in talkIDs)
{
    var newTalkInterest = new TalkInterest
    {
        TalkID = talk,
        InterestedRegistrantID = registrantID
    };
    talkInterests.Add(newTalkInterest);
}

talkInterests.Dump();

TalkInterests.InsertAllOnSubmit(talkInterests);
